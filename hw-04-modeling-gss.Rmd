...
gss16_1 <- gss16 %>% 
  mutate(advfront = 
           if_else(advfront==("Strongly agree") | advfront==("Agree"), "Agree", "Not agree"))
```
...
gss16_2 <- gss16_1 %>% 
  mutate(polviews = 
           if_else(str_detect(polviews, "[Ll]iberal"), "Liberal", 
                   if_else(str_detect(polviews, "[Cc]onservative"), "Conservative",
                           "Moderate")), 
...
***I cannot tell which one works better because collect_metrics isn't working, but whichever one had a larger area under the ROC curve would be preforming better.***
...
gss16_3 <- gss16 %>% 
  mutate(harass5 = 
           if_else(harass5==("Yes") | harass5==("No"), harass5, NA)) %>%
  drop_na(harass5)
...
***We would be resampling from the same dataset (gss16_3) with different seeds, so each sample acts like a different dataset.***
...
***We are 95% confident that between 15.44064% and 18.94028% of Americans have been harassed by their co-workers or superiors at their job.***
...
***I would expect it to be narrower, because 90% has to adress less of a chance of differences, while 95% has to cover that extra 5%.***
